...
function slideShow(){
  ...
  for(x = 0; x < simg.length; x++){
    simg[x].style.display = "none";
  }
  indexValue++;
  if(indexValue > simg.length){indexValue = 1}
  simg[indexValue -1].style.display = "block";
}
slideShow();
...
function renderclothes(pic, productName, brandName, priceTag) {
  ...
  price.innerHTML = "Rs" + " " + priceTag;
  card.appendChild(price);

  var cards = document.getElementsByClassName("clothing-card")[0];
...
function renderaccessories(pic, productName, brandName, priceTag) {
  var card = document.createElement("div");
  card.classList.add("card");
  var img = document.createElement("img");
  img.classList.add("img");
  ...
  var cards = document.getElementsByClassName("accessories-card")[0];
  cards.appendChild(card);
}
for (var i = 5; i < 10; i++) {
  renderaccessories(
